...
//角色
app.controller("RoleController", ['localStorageService', '$scope', function (localStorageService, $scope) {
    var self = this;
    var propertyName = "RoleList";
    self.isList = true;//列表模式flag
    ...
    self.submit = function (ability) {
        ...
        if (copy.id) {
            angular.array.forEach(self.dataList, function (value, key) {
                if (value.id == copy.id)
    ...
    //確認是否可以儲存
    self.checkForm = function (checkAbiltySet) {
        if (!self.editData.name)
            return false;
        return checkAbiltySet();
    };
}]);
